...
printf 'hello' > "${ROOT_DIR}/peer1/identity-source.txt"

./build/server/p2p-server "${PORT}" > "${ROOT_DIR}/server.log" 2>&1 &
SERVER_PID=$!
sleep 1

(sleep 8; printf 'quit\n') | ./build/client/p2p-client 127.0.0.1 "${PORT}" 7211 "${ROOT_DIR}/peer1" \
...
printf 'find -s 5 11831194018420276491\nrequest 5 11831194018420276491\nquit\n' | \
    ./build/client/p2p-client 127.0.0.1 "${PORT}" 7212 "${ROOT_DIR}/peer2" \
    > "${ROOT_DIR}/client2.log" 2>&1

require_log "identity-source.txt" "${ROOT_DIR}/client2.log"
require_log "127.0.0.1:7211" "${ROOT_DIR}/client2.log"
...
cmp "${ROOT_DIR}/peer1/identity-source.txt" "${DOWNLOADED}" >/dev/null

echo "test_identity_request_transfer: ok"
